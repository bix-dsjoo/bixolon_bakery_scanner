...
- A Stage-5 schedule is derived only from hash-bound Stage-4 confirmation receipts for one method, selector, fold, and support seed; the certificate proves the immediate preregistered predecessor failure, provisional minimum, next passing anchor, and balanced 150-shot reference. It also binds every direct Stage-2/3 scorer receipt in that ascending lineage (including the mandatory 150-shot ascending reference), re-derives each from raw evidence, and checks the claimed pass/fail result. The preregistered `k=1/3/150` (no lower failure) and `k=40/80/150` (`k=150` is both next anchor and reference) boundary cases use three confirmation receipts. Nested JSON condition fields are re-parsed against their deterministic `condition_id` before either a condition or score receipt is trusted.
