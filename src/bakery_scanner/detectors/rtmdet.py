...
from bakery_scanner.detectors.dfine import _default_gpu_probe, _parse_xyxy, _subprocess_runner, _require_rtx_5080
...
class RTMDetRunner:
    ...
    def train(self, config: str | Path, output: str | Path, *, device: str = "cuda:0") -> dict[str, Any]:
        ...
        return self._command_runner(("rtmdet-train", "--config", str(config), "--work-dir", str(output), "--device", device))
    ...
    def predict(self, model: str | Path, image: str | Path, *, image_id: int, image_size: tuple[int, int], source: str) -> tuple[BreadProposal, ...]:
        ...
        payload = self._command_runner(("rtmdet-predict", "--model", str(model), "--image", str(image), "--device", "cuda:0"))
    ...
    def export_onnx(self, model: str | Path, output: str | Path) -> dict[str, Any]:
        _require_rtx_5080("cuda:0", self._gpu_probe)
        return self._command_runner(("rtmdet-export-onnx", "--model", str(model), "--output", str(output), "--device", "cuda:0"))
