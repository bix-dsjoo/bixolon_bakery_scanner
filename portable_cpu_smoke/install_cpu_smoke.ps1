...
Invoke-Checked 'pip upgrade' @('-m', 'pip', 'install', '--upgrade', 'pip')
Invoke-Checked 'CPU PyTorch installation' @('-m', 'pip', 'install', '--index-url', 'https://download.pytorch.org/whl/cpu', 'torch==2.13.0+cpu', 'torchvision==0.28.0+cpu')
...
Invoke-Checked 'remaining CPU dependency installation' (@('-m', 'pip', 'install') + $otherRequirements)
Invoke-Checked 'local project installation' @('-m', 'pip', 'install', '--no-deps', $root)
